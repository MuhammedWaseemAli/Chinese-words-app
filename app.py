...
# ── Session state ──────────────────────────────────────────────────────────────
defaults = {
    'mode': 'learn',           # 'learn' | 'quiz' | 'speech' | 'progress'
    ...
    'audio_counter': 0,        # incremented to force audio re-render
}
...
# ── Helper: generate & embed audio ────────────────────────────────────────────
def play_audio(text: str, slow: bool = False):
    """Generate TTS and embed an autoplay audio element."""
    try:
        ...
        # unique dummy query-param forces browser to treat it as a new resource
        ts = st.session_state.audio_counter
        st.markdown(
            f'<audio autoplay controls style="width:100%;margin-top:8px">'
            f'<source src="data:audio/mp3;base64,{b64}?v={ts}" type="audio/mp3">'
            f'</audio>',
            unsafe_allow_html=True,
        )
...
def bump_audio():
    """Call before play_audio to make Streamlit re-render the element."""
...
accuracy    = round((st.session_state.quiz_score / max(st.session_state.quiz_total, 1)) * 100, 1) \
              if st.session_state.quiz_total > 0 else 0.0

c1, c2, c3, c4, c5 = st.columns(5)
c1.metric("Total Words",       total_words)
c2.metric("Categories",        categories)
c3.metric("Speech Sentences",  speech_rows)
c4.metric("Quiz Attempts",     st.session_state.quiz_total)
        ...
        for i, row in filtered.iterrows():
            with st.container():
                ...
                if st.button("🔊 Listen", key=f"learn_audio_{i}"):
                    bump_audio()
                    play_audio(row["Traditional Chinese Word"])

    st.divider()
    if st.button("🎲 Random Word"):
        rw = df.sample(1).iloc[0]
        st.info(f"**{rw['Traditional Chinese Word']}** — {rw['English Word']} *({rw['Pinyin']})*  \nCategory: {rw['Category']}")
        bump_audio()
        play_audio(rw["Traditional Chinese Word"])
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if q:
        st.markdown(f"### What is the meaning of:")
        st.markdown(f"<div class='chinese' style='text-align:center;font-size:3rem'>{q['Traditional Chinese Word']}</div>", unsafe_allow_html=True)
        st.caption(f"Pinyin: {q['Pinyin']}")

        if st.button("🔊 Play Pronunciation", key="quiz_play"):
            bump_audio()

        # Always render audio if counter says to
        play_audio(q["Traditional Chinese Word"])

        st.markdown("**Choose the correct translation:**")

        if len(st.session_state.quiz_options) <= 3:
            opt_cols = st.columns(len(st.session_state.quiz_options))
        else:
            ...
            with opt_cols[idx % len(opt_cols)]:
                if st.button(opt, key=f"opt_{idx}_{opt}", disabled=st.session_state.quiz_answered):
                    ...
                    if opt == st.session_state.correct_answer:
                        ...
                        st.error(f"❌ Wrong! Answer: **{st.session_state.correct_answer}**")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("🎲 Generate Speech"):
        ...
        if speech_df.empty:
            st.error("No speech sentences found (Category must contain 'speech').")
        else:
    ...
    if speech:
        for i, s in enumerate(speech):
            with st.container():
                ...
                if st.button(f"🔊 Play sentence {i+1}", key=f"sp_{i}_{st.session_state.audio_counter}"):
                    bump_audio()
                    play_audio(s["Traditional Chinese Word"], slow=(speed == "slow"))

        st.divider()
        if st.button("🔊 Play Full Speech"):
            bump_audio()
            full_text = " ... ".join(s["Traditional Chinese Word"] for s in speech)
            play_audio(full_text, slow=(speed == "slow"))
    else:
